...
exports.protect = catchAsync(async (req, res, next) => {
  // 1. Getting token and check of is is there
  let token;
  if (
  ...
  if (currentUser.isPasswordModifiedAfter(decoded.iat)) {
    return next(
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  if (!user) {
    return next(new AppError('This is email does not exist', 404));
  }
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  // 2. If the token has not expired, and there's a user, set new password
  if (!user) {
